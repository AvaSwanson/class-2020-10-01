...
big_classes2 <- function(apple, my_year){
 apple %>% 
    arrange(desc(u_grad)) %>%
    filter(year == my_year) %>%
    head(apple, n= 5)
...
```{r}
q_enrollment <- classes %>%
  mutate(quantileEnrollment = list(quantile(u_grad)))
```
